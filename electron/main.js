...
const _bootLogPaths = [
  path.join(os.tmpdir(), 'CrystalSpKMacro-startup.log'),
]
if (process.env.PORTABLE_EXECUTABLE_DIR) {
  _bootLogPaths.push(path.join(process.env.PORTABLE_EXECUTABLE_DIR, 'CrystalSpKMacro-startup.log'))
}
...
// AUTH BYPASS: runtime is always allowed
...
async function _downloadPortableUpdate(url, targetPath) {
  const res = await fetch(url, {
    headers: { 'User-Agent': 'CrystalSpKMacro-Updater' },
  ...
  try {
    ...
    const res = await fetch(api, {
      headers: {
        ...
        'User-Agent': 'CrystalSpKMacro-Updater',
      },
    ...
    const expectedAsset = `CrystalSpKMacro-${latestVersion}.exe`
...
// AUTH BYPASS: _setRuntimeUnlocked always keeps runtime unlocked
function _setRuntimeUnlocked(unlocked, reason = '') {
  // Always force unlocked regardless of what the caller requests
  _runtimeUnlocked = true
  _runtimeLeaseUntil = Number.MAX_SAFE_INTEGER
  engine.setRuntimeGuard(_runtimeAllowed)
  if (_latestMacroConfig) {
...
// AUTH BYPASS: _refreshRuntimeLease always succeeds without contacting server
...
function createWindow() {
  ...
  win.webContents.once('did-finish-load', async () => {
    setTimeout(() => {
      ...
      if (!win.isDestroyed()) {
        win.webContents.send(
          'update-error',
          'Input driver failed to load. Reinstall CrystalSpKMacro and Microsoft Visual C++ 2015-2022 x64.'
        )
      }
      try {
        dialog.showMessageBox(win, {
          ...
          message: 'CrystalSpKMacro launched in safe mode.',
          detail: 'Native input driver could not load on this PC. Reinstall CrystalSpKMacro and Microsoft Visual C++ 2015-2022 x64 runtime.',
          buttons: ['OK'],
          noLink: true,
        })
      } catch (_) {}
    ...
    // ── AUTH BYPASS: auto-login immediately without server validation ────────────
    const cached = getStoredLicense()
    win.webContents.send('license-refreshed', { ...cached })
  ...
  // AUTH BYPASS: periodic re-validation disabled (no server calls)
  _revalTimer = setInterval(async () => {
    if (!win || win.isDestroyed()) return
    // Keep runtime unlocked silently
    _setRuntimeUnlocked(true)
  }, 5 * 60 * 1000)
...
ipcMain.handle('clear-license', () => {
  clearLicense()
  // AUTH BYPASS: keep runtime unlocked even after user-initiated logout
  _runtimeUnlocked = true
  _runtimeLeaseUntil = Number.MAX_SAFE_INTEGER
  engine.setRuntimeGuard(_runtimeAllowed)
  return true
...
function _isAllowedExternalUrl(rawUrl) {
  ...
  try {
    ...
    const allowedHosts = new Set([
      'crystalspkmacro.store',
      'www.crystalspkmacro.store',
      'shop.crystalspkmacro.store',
      'discord.gg',
      'modrinth.com',
      'www.modrinth.com'
    ])
...
function _clearRuntimeSessionState(reason = 'panic key triggered') {
  // AUTH BYPASS: do not actually lock runtime; only stop macros for panic
...
// EnumWindows and process scanners see "RuntimeHelper", not "CrystalSpKMacro".
...
app.whenReady().then(() => {
  ...
  // AUTH BYPASS: unlock immediately on startup
  _setRuntimeUnlocked(true)
  createWindow()

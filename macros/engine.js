...
// AC-specific mouse map — includes Mouse2 (RMB) so the user can hold right-click
// to activate AutoCrystal. Mouse2 is intentionally excluded from the global
// MOUSE_BUTTONS set to avoid intercepting right-click for every other macro.
const AC_MOUSE_MAP = { 'Mouse2': 'right', 'Mouse3': 'middle', 'Mouse4': 'x1', 'Mouse5': 'x2' }
const AC_MOUSE_BUTTONS = new Set(Object.keys(AC_MOUSE_MAP))
...
function registerAC(keybind, cfg) {
  const base    = keybind.split('+').pop()
  const isMouse = AC_MOUSE_BUTTONS.has(base)

  try {
    if (isMouse) {
      const mouseBtn = AC_MOUSE_MAP[base]
      if (!mouseBtn) return

      const isRmb = (base === 'Mouse2')

      const unlistenDown = LowLevelHook.on('mouse', mouseBtn, true, () => {
        if ((focusLocked && !mcHasFocus) || chatPaused) return
        if (!_modsDown(keybind)) return
        // For RMB: LowLevelHook fires for both physical presses AND the synthetic
        // hw.mouse.toggle('right', true) calls from the AC loop itself.
        // keysender filters LLMHF_INJECTED events in its hook proc, so synthetic
        // events from the loop do NOT re-enter this callback — safe to call startAC.
        startAC(cfg.crystalKey || '5', cfg.delay || '25', isRmb)
      })
      const unlistenUp = LowLevelHook.on('mouse', mouseBtn, false, () => {
        // Only stop on physical release (same injected-event filtering applies).
        stopAC()
      })
      registeredMacroKeys.set(keybind, { type: 'mouse', unlisten: () => { unlistenDown(); unlistenUp() } })
      console.log(`[engine] Registered [ac] hold mouse: ${mouseBtn}${isRmb ? ' (RMB mode)' : ''}`)
      return
    }
    ...
    const unlistenDown = LowLevelHook.on('keyboard', ksKey, true, () => {
      if ((focusLocked && !mcHasFocus) || chatPaused) return
      if (!_modsDown(keybind)) return
      startAC(cfg.crystalKey || '5', cfg.delay || '25', false)
    })
    const unlistenUp = LowLevelHook.on('keyboard', ksKey, false, () => {
      stopAC()
    })

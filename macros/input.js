...
// ── Auto Crystal (hold-to-run) ────────────────────────────────────────────────
// Behaviour:
//   • Hold any key/button → switches to crystalKey slot once, then loops:
//       rClickFixed (place crystal) → sleep(d) → lClickFixed (hit crystal)
//   • RMB mode (rmbMode=true, keybind=Mouse2):
//       The physical RMB mousedown already fires one right-click in MC.
//       We skip the first synthetic rClick to avoid a double-place race,
//       then continue the normal place→hit loop. This makes the macro feel
//       instant and natural — exactly like holding right-click in vanilla.
//   • Release hold → stopAC() cancels the loop.
let _acActive = false
let _acToken  = null

function startAC(crystalKey, delay, rmbMode) {
  if (_acActive) return
  _acActive = true
  ...
  const isRmb   = !!rmbMode
  console.log(`[macro:ac] started${isRmb ? ' (RMB hold mode)' : ''}`)
  ...
  ;(async () => {
    try {
      ...
      // Switch to crystal slot once on hold start.
      await pressBare(slotKey, tok, SLOT_HOLD_MS)
      if (tok.cancelled || !_acActive) return

      // RMB mode: the physical RMB that triggered this hook already sent one
      // right-click to MC. Wait for MC to process it, then jump straight to
      // left-click (hit the crystal). From there, alternate normally.
      if (isRmb) {
        await sleep(d, tok)
        if (tok.cancelled || !_acActive) return
      ...
      // Main loop: place (rClick) → wait → hit (lClick) → wait → repeat.
